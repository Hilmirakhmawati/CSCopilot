...
export default function KnowledgePage() {
  ...
  async function syncNotion() {
    ...
    try {
      const { data: session } = await getSupabasePublic().auth.getSession();
      const headers = new Headers({ "Content-Type": "application/json" });
      if (session.session?.access_token) headers.set("Authorization", `Bearer ${session.session.access_token}`);
      const response = await fetch("/api/admin/notion/sync", { method: "POST", headers, body: "{}" });
      const data = await response.json();
      if (!response.ok || data.error) throw new Error(data.error || "Notion sync failed");
  ...
  useEffect(() => {
    getSupabasePublic().auth.getSession().then(({ data: session }) => {
      ...
      if (session.session?.access_token) headers.set("Authorization", `Bearer ${session.session.access_token}`);
      return fetch("/api/knowledge", { headers });
      ...
      setItems(mapped);
    }).catch(() => setError("Could not load knowledge")).finally(() => setLoading(false));
  }, []);
