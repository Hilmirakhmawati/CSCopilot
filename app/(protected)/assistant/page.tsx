...
export default function AssistantPage() {
  ...
  async function sendMessage() {
    ...
    try {
      ...
      setAnswer(result); setDraft(result.draft_reply ?? ""); setDraftId(null); setMessages((items) => [...items, { id: result.message_id, role: "assistant", content: result.answer, citations: result.citations ?? [] }]);
  ...
  return <div className="flex h-screen overflow-hidden bg-background text-foreground">
    ...
    <main className="flex min-w-0 flex-1 flex-col">
      ...
      <div className="flex-1 overflow-y-auto py-8"><div className="mx-auto flex max-w-[760px] flex-col gap-[22px] px-6">
        ...
        {messages.map((message) => <div key={message.id} className={`flex max-w-[88%] gap-3 ${message.role === "user" ? "self-end flex-row-reverse" : "self-start"}`}><div className={`grid h-[30px] w-[30px] shrink-0 place-items-center rounded-lg text-sm font-bold ${message.role === "user" ? "bg-slate-200 text-slate-700" : "bg-primary text-white"}`}>{message.role === "user" ? "CS" : "✦"}</div><div className={`rounded-[14px] border px-4 py-3.5 leading-7 ${message.role === "user" ? "rounded-tr-[4px] border-primary bg-primary text-white" : "rounded-tl-[4px] bg-white"}`}><p className="whitespace-pre-line">{message.content}</p>{message.citations?.length ? <div className="mt-3 grid gap-1.5">{message.citations.map((citation) => <a key={`${message.id}-${citation.document_id}`} href={citation.url ?? "#"} target="_blank" rel="noreferrer" className="flex items-center gap-2 rounded-lg border bg-slate-50 px-2.5 py-2 text-xs text-muted-foreground"><FileText className="h-3.5 w-3.5" /><b className="truncate text-foreground">{citation.title}</b><Badge className="ml-auto shrink-0 bg-green-100 text-green-700 hover:bg-green-100">Source</Badge></a>)}</div> : null}</div></div>)}
        ...
        {draft && answer && <div className="ml-[42px] max-w-[calc(88%-42px)] rounded-xl border border-primary/20 bg-primary/5 p-4"><div className="mb-2 flex items-center justify-between gap-3"><p className="text-xs font-bold uppercase tracking-wider text-primary">Suggested reply</p><Button variant="ghost" size="sm" onClick={() => void copyDraft()}>{copied ? <Check className="h-4 w-4 text-emerald-600" /> : <Clipboard className="h-4 w-4" />}{copied ? "Copied" : "Copy"}</Button></div><Textarea value={draft} onChange={(event) => setDraft(event.target.value)} className="min-h-24 resize-y bg-white" /><div className="mt-3 flex gap-2"><Button variant="outline" className="flex-1" onClick={() => void saveDraft()}>Save draft</Button><Button className="flex-1" disabled={approved} onClick={() => void approve()}>{approved ? "Approved" : "Approve draft"}</Button></div></div>}
        <div ref={bottomRef} />
      </div></div>
