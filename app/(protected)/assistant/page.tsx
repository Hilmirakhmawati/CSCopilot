...
const suggestions = [
  "Kenapa saldo poin 0?",
  "Kenapa riwayat poin beda?",
  "Cara cek perhitungan poin?",
  "Kenapa riwayat poin tidak sesuai?",
  "Cara audit saldo poin?",
];
...
export default function AssistantPage() {
  ...
  async function approve() {
    if (approving || approved || rejected || !draft.trim() || answer?.missing_context?.length) return;
    setApproving(true);
    try {
  ...
  async function reject() {
    if (rejecting || approved || rejected || !draft.trim()) return;
  ...
  return <div className="flex h-dvh overflow-hidden bg-background text-foreground">
    ...
    <main className="flex min-w-0 flex-1 flex-col">
      ...
      <div className="flex-1 overflow-y-auto py-8"><div className="mx-auto flex max-w-[760px] flex-col gap-[22px] px-6">
        ...
        {answer && answer.intent !== "Greeting" && answer.recommended_action && answer.missing_context?.length ? <div className="ml-[42px] max-w-[calc(88%-42px)] rounded-lg border border-slate-200 bg-slate-50 px-3.5 py-3 text-[13px] text-slate-700"><b className="mb-0.5 block text-slate-900">Instruksi internal (tidak untuk dikirim ke customer)</b><p className="whitespace-pre-line leading-6">{answer.recommended_action}</p></div> : null}
        {answer?.knowledge_gap ? <div className="ml-[42px] max-w-[calc(88%-42px)] rounded-lg border border-red-200 bg-red-50 px-3.5 py-3 text-[13px] text-red-800"><b className="mb-0.5 block text-red-900">Knowledge belum lengkap</b><p className="leading-6">{answer.missing_context.join(" ")}</p><p className="mt-1 text-xs text-red-700">Lengkapi field <b>Customer Reply</b> di Notion, lalu lakukan sync ulang.</p></div> : null}
        {answer?.missing_context?.length && !answer.knowledge_gap ? <div className="ml-[42px] max-w-[calc(88%-42px)] rounded-lg border border-amber-200 bg-amber-50 px-3.5 py-3 text-[13px] text-amber-800"><b className="mb-0.5 block text-amber-900">Data customer diperlukan</b><p className="leading-6">{answer.missing_context.join(" ")}</p></div> : null}
